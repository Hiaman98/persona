...
export function useChat() {
  ...
  const formatMessagesForClaude = (chatMessages, activePersonaId) => {
    const formatted = chatMessages
      .filter((m) => m.text && m.text.trim()) // skip empty text/placeholders
      .map((m) => {
        if (m.role === "assistant") {
          const msgPersonaId = m.personaId || (m.avatar === "PG" ? "piyush" : "hitesh");
          const isDifferentPersona = msgPersonaId !== activePersonaId;
          
          if (isDifferentPersona) {
            const senderName = PERSONAS[msgPersonaId]?.name || "Another Persona";
            return {
              role: m.role,
              content: `[Response from ${senderName} - Note: This was generated by a different simulated persona. Ignore this style and tone]: ${m.text}`,
            };
          }
        }
        return {
  ...
  const sendMessage = (text, personaId) => {
    ...
    const apiMessages = formatMessagesForClaude([...activeChat.messages, userMsg], personaId);
